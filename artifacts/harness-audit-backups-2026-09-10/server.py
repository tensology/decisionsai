...
def create_app() -> FastAPI:
    ...
    try:
        from distr.gui.web.routes.speech_injector import router as speech_injector_router

        app.include_router(speech_injector_router)
        logger.info("Speech injector mounted at POST /api/speech/inject")
    except Exception as e:
        logger.error("Failed to load speech injector route: %s", e, exc_info=True)
    ...
    async def projects_page(request: Request):
        return RedirectResponse(url="/development/", status_code=302)
    ...
    async def development_page(request: Request):
        return page_templates.TemplateResponse(
            ...
            _template_context(request, "/development"),
        )

    @app.get("/development", response_class=HTMLResponse)
    async def development_redirect():
        return RedirectResponse(url="/development/", status_code=302)

    @app.get("/development/{development_path:path}", response_class=HTMLResponse)
    async def development_deep_link(request: Request, development_path: str):
        return page_templates.TemplateResponse(
            request,
            "workflows/studio.html",
            _template_context(request, "/development"),
    ...
    async def workflows_page(request: Request):
        template = (
            ...
            if request.query_params.get("mode") == "blueprint"
            else "workflows/studio.html"
        )
        return page_templates.TemplateResponse(request, template, _template_context(request, "/workflows"))
    ...
    async def intake_legacy_redirect():
        return RedirectResponse(url="/development/incoming/", status_code=302)

    @app.get("/intake", response_class=HTMLResponse)
    async def intake_redirect():
        return RedirectResponse(url="/development/incoming/", status_code=302)

    def _legacy_ticket_destination(request: Request) -> str:
        """Keep old bookmarks working while retiring the duplicate Tickets UI."""
        source = (request.query_params.get("source") or "database").strip().lower()
        board_id = (request.query_params.get("board_id") or "").strip()
        ticket_id = (request.query_params.get("ticket_id") or "").strip()
        if not board_id and ticket_id.isdigit():
            try:
                from distr.core.db import get_session
                ...
                with get_session() as session:
                    ...
                    board_id = str(row[0]) if row else ""
            except Exception:
                logger.debug("Could not resolve legacy ticket deep link", exc_info=True)
        if board_id:
            provider = source if source in {"jira", "trello"} else "decisions"
            suffix = f"?ticket_id={ticket_id}" if ticket_id.isdigit() else ""
            if request.query_params.get("view") == "list":
                suffix += ("&" if suffix else "?") + "view=list"
            return f"/development/boards/{provider}/{board_id}/kanban/{suffix}"
        return "/development/"

    # The standalone Tickets page has been folded into Development. Preserve
    # these routes only as compatibility redirects for old bookmarks and tools.
    @app.get("/tickets/", response_class=HTMLResponse)
    async def tickets_page(request: Request):
        return RedirectResponse(url=_legacy_ticket_destination(request), status_code=302)

    @app.get("/tickets", response_class=HTMLResponse)
    async def tickets_redirect(request: Request):
        return RedirectResponse(url=_legacy_ticket_destination(request), status_code=302)

    @app.get("/kanban/", response_class=HTMLResponse)
    async def kanban_legacy_page_redirect(request: Request):
        return RedirectResponse(url=_legacy_ticket_destination(request), status_code=302)

    @app.get("/kanban", response_class=HTMLResponse)
    async def kanban_legacy_redirect(request: Request):
        return RedirectResponse(url=_legacy_ticket_destination(request), status_code=302)
    ...
    @app.on_event("startup")
    async def _select_free_auto_development_model():
        """Resolve Auto to a concrete free leaderboard model for this launch."""
        try:
            import asyncio
            ...
            selected = await asyncio.to_thread(refresh_global_auto_model_policy)
            primary = selected.get("primary") or {}
            fallback = selected.get("fallback") or {}
            logger.info(
                "Development Auto selected free model %s/%s with fallback %s/%s",
                primary.get("model_provider"),
                primary.get("model"),
                fallback.get("model_provider"),
                fallback.get("model"),
            )
        except Exception as e:
            logger.warning("Could not refresh the free Development Auto route: %s", e)
