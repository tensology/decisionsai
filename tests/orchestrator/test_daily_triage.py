...
from distr.core.orchestrator_daily_triage import (
    build_daily_triage,
...
def test_enqueue_triage_candidates_dedupes(tmp_path):
    ...
    assert entries[0].action_type == "orchestrator_triage_candidate"
    assert entries[0].execute_payload["kind"] == "orchestrator_triage_ack"
...
def test_orchestrator_triage_ack_executes_whatsapp_ticket_when_linked(monkeypatch):
    ...
    monkeypatch.setattr("distr.core.orchestrator.emit_event", lambda **kw: events.append(kw))

    draft_execute.run_execute_payload({
        "kind": "orchestrator_triage_ack",
        "candidate": {
            "source": "whatsapp",
