...
def workspace_item(workspace, item_id):
    return next(item for item in planning.get_workspace(workspace["id"])["items"] if item["id"] == item_id)
...
def test_database_commit_failure_cannot_change_the_project_file(plan_workspace_db, monkeypatch):
    ...
    assert workspace_item(workspace, item["id"])["content"] == "Original"
...
def test_failed_file_projection_recovers_after_the_committed_revision(plan_workspace_db, monkeypatch):
    ...
    with monkeypatch.context() as patch:
        ...
        assert Path(item["file_path"]).read_text() == "Original"
    # Reads are pure. Reconciliation is an explicit write-side operation.
    recovered_items = planning.reconcile_workspace(workspace["id"])["items"]
    recovered = next(row for row in recovered_items if row["id"] == item["id"])
