...
from distr.core.hermes_validator import (
    _parse_pass_fail_response,
    apply_hermes_validator_overlay,
    run_hermes_validator_judgment,
)
...
@patch("distr.core.hermes_validator.run_hermes_validator_judgment")
@patch("distr.core.hermes_validator.is_hermes_validator_second_pass_enabled", return_value=True)
def test_apply_hermes_validator_overlay_fails_closed(mock_enabled, mock_judge, mock_emit):
    ...
    overlay = apply_hermes_validator_overlay(
        step=step,
        result="done",
        caller_passed=True,
...
@patch("distr.core.hermes_validator.is_hermes_validator_second_pass_enabled", return_value=True)
def test_apply_hermes_validator_overlay_skips_when_mechanical_failed(mock_enabled):
    step = MagicMock()
