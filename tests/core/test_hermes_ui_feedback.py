...
def test_visual_taste_summary_aggregates_ui_feedback_labels(tmp_path):
    ...
    from distr.core.hermes import build_visual_taste_summary, record_ui_feedback_label
...
def test_visual_taste_context_formats_reusable_preferences(tmp_path):
    ...
    from distr.core.hermes import build_visual_taste_context, record_ui_feedback_label
...
def test_board_activity_includes_visual_taste_summary(tmp_path):
    ...
    from distr.core.hermes import list_board_activity, record_ui_feedback_label
...
def test_record_ui_quality_validation_fails_when_artifacts_missing(tmp_path):
    ...
    from distr.core.hermes import record_ui_quality_validation
    ...
    with patch("distr.core.hermes.get_session", get_session), patch("distr.core.db.get_session", get_session):
        ...
        with get_session() as session:
            validation = (
                ...
                .filter(HermesValidationRecord.id == record_id)
                .one()
            )
...
def test_ui_quality_validation_requires_checks_for_repeated_rejection_reasons(tmp_path):
    ...
    with patch("distr.core.hermes.get_session", get_session), patch("distr.core.db.get_session", get_session):
        ...
        with get_session() as session:
            failed = (
                session.query(HermesValidationRecord)
                .filter(HermesValidationRecord.id == failed_id)
                .one()
            )
            passed = (
                session.query(HermesValidationRecord)
                .filter(HermesValidationRecord.id == passed_id)
                .one()
            )
...
def test_visual_baseline_upsert_adds_and_replaces_screens_in_existing_set(tmp_path):
    ...
    from distr.core.hermes import get_visual_baseline_set, upsert_visual_baseline_screens
...
def test_visual_baseline_readiness_reports_missing_screenshot_files(tmp_path):
    ...
    from distr.core.hermes import create_visual_baseline_set, inspect_visual_baseline_readiness
...
def test_ui_quality_validation_records_visual_baseline_failure(tmp_path):
    ...
    from distr.core.db.hermes import HermesValidationRecord
    from distr.core.hermes import create_visual_baseline_set, record_ui_quality_validation
...
def test_approved_baseline_copy_feeds_later_ui_validation(tmp_path):
    ...
    with patch("distr.core.hermes.get_session", get_session), patch("distr.core.db.get_session", get_session):
        ...
        with get_session() as session:
            ...
            failed_validation = (
                session.query(HermesValidationRecord)
                .filter(HermesValidationRecord.id == failed_record_id)
                .one()
            )
