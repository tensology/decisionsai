...
def test_chat_service_user_message_records_hermes_audit_event(monkeypatch):
    ...
    with _session_ctx(factory) as session:
        events = session.query(OrchestratorEvent).all()
        assert len(events) == 1
        event = events[0]
...
def test_removing_chat_transcript_audit_events_preserves_hermes_memory(monkeypatch):
    ...
    with _session_ctx(factory) as session:
        assert session.query(OrchestratorEvent).filter(OrchestratorEvent.source == "chat").count() == 2
        assert session.query(OrchestratorEvent).filter(OrchestratorEvent.source == "orchestrator_learning").count() == 1
        assert session.query(OrchestratorLearnedRule).count() == 1

    deleted = remove_chat_transcript_audit_events(chat_id)

    with _session_ctx(factory) as session:
        assert deleted == 1
        assert session.query(OrchestratorEvent).filter(OrchestratorEvent.source == "chat").count() == 1
        assert session.query(OrchestratorEvent).filter(OrchestratorEvent.source == "orchestrator_learning").count() == 1
        assert session.query(OrchestratorLearnedRule).count() == 1
...
def test_chat_manager_clear_removes_transcript_audit_events_only(monkeypatch):
    ...
    with _session_ctx(factory) as session:
        ...
        assert session.query(OrchestratorEvent).filter(OrchestratorEvent.source == "chat").count() == 1
        remaining = session.query(OrchestratorEvent).filter(OrchestratorEvent.source == "chat").one()
