...
from distr.core.db.orchestrator import OrchestratorEvent, OrchestratorMachineActivity, OrchestratorUserMemory
...
def test_user_memory_upserts_duplicates_and_emits_memory_event(monkeypatch):
    from distr.core.orchestrator_memory import list_user_memories, record_user_memory
    ...
    with _session_ctx(factory) as session:
        memories = session.query(OrchestratorUserMemory).all()
        events = session.query(OrchestratorEvent).filter(OrchestratorEvent.event_type == "memory_written").all()
...
def test_chat_memory_extraction_survives_chat_clear(monkeypatch):
    ...
    with _session_ctx(factory) as session:
        transcript_events = session.query(OrchestratorEvent).filter(OrchestratorEvent.source == "chat").all()
        memories = session.query(OrchestratorUserMemory).all()
...
def test_memory_extraction_records_idle_voice_note_guardrail():
    from distr.core.orchestrator_memory import extract_memory_candidates_from_text
...
def test_machine_activity_records_quiet_local_context(monkeypatch):
    from distr.core.orchestrator_memory import list_machine_activity, record_machine_activity
    ...
    with _session_ctx(factory) as session:
        rows = session.query(OrchestratorMachineActivity).all()
        notification_events = session.query(OrchestratorEvent).filter(OrchestratorEvent.source == "notification").all()
...
def test_machine_activity_compaction_summarizes_old_detail_without_notifications(monkeypatch):
    from distr.core.orchestrator_memory import compact_machine_activity, list_machine_activity, record_machine_activity
...
def test_orchestrator_memory_routes_expose_memories_and_activity(monkeypatch):
    from distr.core.orchestrator_memory import record_machine_activity
    from distr.gui.web.routes.orchestrator_memory import create_routes
    ...
    memories = client.get("/api/orchestrator/memories").json()
    activity = client.get("/api/orchestrator/activity").json()
