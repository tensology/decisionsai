...
def test_delegated_workflow_tool_returns_plan_and_records_event(monkeypatch):
    from distr.core.agent.tools.integrations.delegated_workflow import DelegatedWorkflowTool

    emitted = []
    monkeypatch.setattr("distr.core.orchestrator.emit_event", lambda **kwargs: emitted.append(kwargs) or 456)

    result = DelegatedWorkflowTool()._run(
...
def test_delegated_workflow_tool_can_execute_with_injected_runner(monkeypatch):
    from distr.core.agent.tools.integrations.delegated_workflow import DelegatedWorkflowTool
    from distr.core.delegated_workflow.models import DelegatedRunReport

    emitted = []
    monkeypatch.setattr("distr.core.orchestrator.emit_event", lambda **kwargs: emitted.append(kwargs) or (789 + len(emitted)))
    ...
    tool = DelegatedWorkflowTool(runner=FakeRunner())
...
def test_delegated_workflow_tool_can_return_preflight_without_execution(monkeypatch):
    from distr.core.agent.tools.integrations.delegated_workflow import DelegatedWorkflowTool

    emitted = []
    monkeypatch.setattr("distr.core.orchestrator.emit_event", lambda **kwargs: emitted.append(kwargs) or (900 + len(emitted)))
