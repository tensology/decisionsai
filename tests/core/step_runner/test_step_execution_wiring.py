...
from distr.app.workflow import WorkflowOrchestrationMixin, _DIRECT_EXECUTION_TYPES
...
class TestValidationBeforeExecution:
    ...
    def test_agent_instruction_skips_validation(self, mock_get_session, mock_build, mock_assemble):
        ...
        # Provide a mock WorkflowAgent and event loop so the agent path runs
        ...
        orch = _make_orch()
        orch["workflow_agent"] = mock_agent
        orch["agent_loop"] = mock_loop

        with patch("asyncio.run_coroutine_threadsafe", return_value=mock_future):
            mixin._send_workflow_instruction(orch, 0)

        # Should NOT be marked as failed (no validation for agent_instruction)
        fail_calls = [c for c in mixin._set_workflow_step_status.call_args_list if len(c[0]) >= 2 and c[0][1] == "failed"]
        assert len(fail_calls) == 0
...
class TestDirectExecutionTypes:
    ...
    def test_direct_execution_types(self):
        assert _DIRECT_EXECUTION_TYPES == {
            "run_command", "http_request", "execute_code", "playwright", "computer_use", "play_recording", "ytdlp"
        }
...
class TestExecRunCommand:
    ...
    def test_successful_command(self):
        result, success = WorkflowOrchestrationMixin._exec_run_command({"command": "echo hello"})
        assert success is True
        assert "hello" in result

    def test_failing_command(self):
        result, success = WorkflowOrchestrationMixin._exec_run_command({"command": "exit 1"})
        assert success is False

    def test_timeout(self):
        result, success = WorkflowOrchestrationMixin._exec_run_command({
            "command": "sleep 10",
            "timeout_seconds": 1,
        })
        assert success is False
    ...
    def test_empty_command(self):
        ...
        result, success = WorkflowOrchestrationMixin._exec_run_command({"command": ""})
...
class TestExecExecuteCode:
    ...
    def test_successful_code(self):
        result, success = WorkflowOrchestrationMixin._exec_execute_code(
...
class TestPlayRecordingExecution:
    ...
    def test_play_by_name(self, mock_signal_manager):
        ...
        # Signal should be emitted to start playback
        mock_signal_manager.play_action_by_name.emit.assert_called_once_with("my_recording")
        # Step should be set to running (waiting for playback to finish)
        running_calls = [c for c in mixin._set_workflow_step_status.call_args_list if len(c[0]) >= 2 and c[0][1] == "running"]
        assert len(running_calls) >= 1
        # Listeners should be connected
        mock_signal_manager.action_playback_finished.connect.assert_called_once()
        mock_signal_manager.action_playback_stopped.connect.assert_called_once()
