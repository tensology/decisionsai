...
# ---------------------------------------------------------------------------
# Mock heavy dependencies BEFORE importing distr.app.step_runner
# ---------------------------------------------------------------------------
_mock_qt = MagicMock()
...
from distr.app.step_runner import StepRunnerMixin, _DIRECT_EXECUTION_TYPES  # noqa: E402
...
# Non-empty printable text for titles, instructions, session instructions
...
_session_ids = st.integers(min_value=1, max_value=10_000)
...
def _insert_orchestration(mixin, session_id, steps, run_id=None):
...
class TestPBTExplorationSignalManagerRouting:
    ...
    ``_send_step_runner_instruction`` routes to ``WorkflowAgent.execute()``
    instead.
    ...
    **Validates: Requirements 1.1**
    ...
    def test_agent_instruction_routes_through_signal_manager(self, session_id, step):
        ...
        mixin = _setup_mixin()
        steps = [step]
        orch, mock_agent, mock_loop = _insert_orchestration(mixin, session_id, steps)

        with patch("distr.app.step_runner.signal_manager") as mock_signals:
            # Provide a prompt override so we skip DB lookups
            mixin._send_step_runner_instruction(orch, 0, prompt="test instruction")
...
class TestPBTFixWorkflowAgentRouting:
    ...
    **Validates: Requirements 2.1, 2.2, 2.4**
    ...
    def test_agent_instruction_routes_to_workflow_agent(
        self, mock_run_coro, session_id, step, instruction
    ):
        ...
        mixin._send_step_runner_instruction(orch, 0, prompt=instruction)
    ...
    def test_signal_manager_never_called(
        self, mock_signals, mock_run_coro, session_id, step
    ):
        ...
        mixin = _setup_mixin()
        steps = [step]
        orch, _, _ = _insert_orchestration(mixin, session_id, steps)
    ...
    def test_concurrent_agents_have_isolated_history(
        ...
        mixin._send_step_runner_instruction(orch_a, 0, prompt="instruction A")
        first_call_loop = mock_run_coro.call_args_list[0][0][1]
        assert first_call_loop is loop_a

        # Sending instruction to B uses B's loop
        mixin._send_step_runner_instruction(orch_b, 0, prompt="instruction B")
...
class TestPBTPreservationDirectExecution:
    ...
    **Validates: Requirements 3.3**
    ...
    def test_direct_types_do_not_route_to_workflow_agent(
        self, mock_run_coro, session_id, step
    ):
        ...
        mock_sess_obj = SimpleNamespace(id=session_id, context_rules=None, workflow_input=None)
        ...
        # Mock _validate_step_config to return True (valid config)
        mixin._validate_step_config = MagicMock(return_value=True)
    ...
    def test_direct_types_do_not_use_signal_manager(
        self, mock_run_coro, mock_signals, session_id, step
    ):
...
class TestPBTPreservationLifecycleStateConsistency:
    ...
    (start, advance, error, cancel, finish), the ``_step_runner_orchestrations``
    ...
    **Validates: Requirements 3.4, 3.5**
    ...
    @settings(max_examples=50, suppress_health_check=[HealthCheck.function_scoped_fixture])
    def test_lifecycle_events_maintain_dict_consistency(self, session_id, steps, events):
        ...
        def mock_finish(session_id, success=True, cancelled=False):
            mixin._cancel_step_runner_timeout(session_id)
            with __import__("distr.app.step_runner", fromlist=["_orch_lock"])._orch_lock if False else threading.Lock():
                orch = mixin._step_runner_orchestrations.pop(session_id, None)
            if orch:
        ...
        mixin._finish_step_runner_orchestration = mock_finish

        for event in events:
            if event == "start":
                if session_id not in mixin._step_runner_orchestrations:
                    _insert_orchestration(mixin, session_id, steps)

            elif event == "advance":
                orch = mixin._step_runner_orchestrations.get(session_id)
                if orch:
                    ...
                    if idx + 1 < len(orch["steps_data"]):
                        ...
                        mock_finish(session_id=session_id, success=True)
                ...
                if orch:
                    ...
                    if orch["retry_count"] >= orch["max_retries"]:
                        ...
                        if idx + 1 < len(orch["steps_data"]):
                            ...
                            mock_finish(session_id=session_id, success=False)

            elif event == "cancel":
                if session_id in mixin._step_runner_orchestrations:
                    mock_finish(session_id=session_id, success=False, cancelled=True)

            elif event == "finish":
                if session_id in mixin._step_runner_orchestrations:
                    orch = mixin._step_runner_orchestrations[session_id]
                    mock_finish(
                        session_id=session_id,
            ...
            for sid, orch in mixin._step_runner_orchestrations.items():
                # Every live orchestration has a valid session_id key
    ...
    def test_finishing_one_orchestration_preserves_others(self, sid_a, sid_b, steps_a, steps_b):
        ...
        # Remove A
        removed = mixin._step_runner_orchestrations.pop(sid_a, None)
        assert removed is orch_a

        # B is still intact
        assert sid_b in mixin._step_runner_orchestrations
        remaining = mixin._step_runner_orchestrations[sid_b]
    ...
    def test_advance_increments_index_correctly(self, session_id, steps, advance_count):
    ...
    @settings(max_examples=30, suppress_health_check=[HealthCheck.function_scoped_fixture])
    def test_cancel_removes_orchestration(self, session_id, steps):
        """Cancelling an orchestration removes it from the dict."""
        mixin = _setup_mixin()
        _insert_orchestration(mixin, session_id, steps)

        assert session_id in mixin._step_runner_orchestrations
