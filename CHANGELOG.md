...
DecisionsAI is an agent that lives on your computer. You can talk to it, type a request, or send it something from your phone. It can work with your files, apps, tools, AI models, and coding programs. A normal chat gives you an answer. DecisionsAI can also keep hold of the job, do the work, and show you what happened.

The quickest way in is the Oracle, a small avatar that stays above your other windows. Hold the talk shortcut and ask it to do something. Use the dictation shortcut when you only want your speech typed into the app in front of you. You can also open Chat and write to the same agent.

When you connect a project folder, DecisionsAI knows where the work belongs. If you say, "make the green order button black," it can attach that request to the project, create a ticket, send the change to a coding tool, and keep the result with the ticket. The ticket gives you a record of the request, the time spent, the files changed, and whether the work passed its checks.

Some jobs are too large for one prompt. A model asked to understand a project, make a plan, write the code, and judge its own work in one go will often miss something. For a larger job, DecisionsAI can use a workflow. It reads the request and project first, makes a focused plan, carries out the work, checks the result in a separate pass, sends failed work back for correction, and finishes with a report. Each step can use the model or coding tool that suits that part of the job. The workflow keeps the instructions focused and gives tests and other evidence a say in whether the work is done. You can see the current step, steer it, or stop it.

The web interface is where you look after projects, tickets, workflows, snippets, and completed runs. You do not need to watch it while the agent works. Telegram gives you access to the same agent from your phone, so a message, voice note, screenshot, document, approval, or correction can stay connected to the job you started on your computer.

Telegram can also open a secure remote control for your own DecisionsAI computer. From your phone you can see the screen, click, type, scroll, use saved snippets, and move files between the two devices. This is useful when the file you need is on the wrong device, or when you want to check something on your computer without sitting in front of it.

WhatsApp handles a different part of the job. You can link a contact or group to a ticket board and let DecisionsAI collect the messages and media that belong to the work. Those messages can become tickets with their conversation context attached. You choose which feeds are connected and how much freedom the agent has, so an ordinary WhatsApp conversation does not automatically become an instruction to change a project.

DecisionsAI does not tie you to one AI company. You can use models on your own computer, connect an API, or send coding work to Codex, Cursor, Claude Code, Pi, and other supported tools. You can change those choices later without handing your project history to one vendor. The models and tools still need to be installed, connected, and capable of doing the job. DecisionsAI checks what is available, tells you what it chose, and should not call failed or unchecked work complete.

You can [download or clone the repository](https://github.com/tensology/decisionsai) and use the codebase directly. Codex can work inside the repository while DecisionsAI keeps the wider job attached to its project and ticket. For work that benefits from another pass, a loop can inspect the result, run a check, send back a specific correction, and repeat until the check passes or you stop it.

For a developer, DecisionsAI is a toolkit rather than another coding chat. Voice and dictation help you capture the request. Projects and tickets keep it organized. Workflows give larger jobs a method. Coding tools do the implementation, while skills, snippets, tests, browser checks, memory, and the phone controls help carry the work through and keep it useful after one conversation ends.

Codex is excellent when you are already inside a repository and want a capable coding agent to handle a task. For one focused code change, using Codex on its own may be the simplest choice. DecisionsAI adds the part around that coding session. A request can arrive through voice, Telegram, WhatsApp, Chat, or a ticket board. DecisionsAI can attach it to the right project, hand the coding work to Codex, ask another worker to check the result, keep the evidence and time with the ticket, and send the outcome back to your phone.

That is why DecisionsAI can be better than using Codex on its own for ongoing development work. It gives separate requests one shared history and a repeatable way to move from an idea to checked work. You can leave it running, see what is happening, step in when needed, and change the models or coding tools later. DecisionsAI is moving toward handling more of this without supervision, but it should still show what happened and ask for your decision when it matters.
