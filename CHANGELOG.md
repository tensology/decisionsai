...
DecisionsAI is a local-first agent and work coordinator. It sits between you and the collection of models, coding CLIs, tools, projects, and communication channels you already use. It is not another foundation model, and it is not a claim that every vague request can safely disappear into a black box. Its job is to understand what you are asking, give the work a proper home, decide how much process it needs, send it to an appropriate worker, keep the history and evidence together, and bring you in when a real decision is required.

On the desktop, the Oracle — a small configurable avatar that stays above your windows — is the quickest way in. Speak to the agent, use a separate hotkey to dictate into another app, type in Chat, or ask it to work with what is on your computer. The local web interface is the control deck behind that conversation: projects hold context, tickets make changes trackable, and workflows give larger jobs a repeatable route through planning, implementation, review, correction, and reporting. A simple request does not need a workflow. A larger or riskier one can use the built-in Development workflow instead of relying on one enormous prompt.

Telegram gives the same agent a remote control on your phone. Text, voice notes, screenshots, documents, approvals, corrections, and progress can stay connected to the same underlying project and run. You do not need to keep the web interface open, but you can return to it when you want to see which ticket is moving, which worker was chosen, what evidence came back, or why the agent stopped to ask you something.

DecisionsAI is model-, provider-, and CLI-agnostic by design. It can route configured work between local models, APIs, Codex, Cursor, Claude Code, Pi, and other workers without making one vendor the permanent owner of the project memory. That does not make every provider equally capable or automatically ready: credentials, installed tools, available models, project boundaries, and validation still matter. The orchestrator's role is to check those conditions, make the route visible, recover or offer another route when it can, and avoid pretending that failed or unverified work is complete.

You can [download or clone the repository](https://github.com/tensology/decisionsai) and use the codebase directly. If you work in Codex, its looping capability can operate inside the repository while DecisionsAI holds the wider project, ticket, workflow, and result around that work. A loop can inspect an outcome, run the relevant check, carry a focused correction into another pass, and stop when the evidence says the goal has been met — with you still able to steer or stop it.

That is what DecisionsAI is trying to become increasingly good at: one dependable place where work arrives, gets understood and organized, moves through the right tools, and comes back with a result you can inspect. The destination is more automated work, not less accountability.
