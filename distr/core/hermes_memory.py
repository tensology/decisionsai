...
from distr.core.db.hermes import HermesMachineActivity, HermesMaintenanceState, HermesUserMemory
...
def ensure_hermes_memory_tables() -> None:
...
def _serialize_memory(row: HermesUserMemory) -> dict[str, Any]:
    return {
        "id": row.id,
...
def record_user_memory(
    ...
    *,
    ...
    ensure_hermes_memory_tables()
    with get_session() as session:
        row = (
            session.query(HermesUserMemory)
            .filter(HermesUserMemory.content_hash == content_hash)
            .filter(HermesUserMemory.category == category_clean)
            .filter(HermesUserMemory.scope == scope_clean)
            .filter(HermesUserMemory.scope_id == scope_id)
            .first()
        )
        if row:
...
def list_user_memories(
    *,
    ...
    ensure_hermes_memory_tables()
    with get_session() as session:
        query = session.query(HermesUserMemory)
        if category:
            query = query.filter(HermesUserMemory.category == category.strip().lower().replace(" ", "_"))
        if not include_disabled:
            query = query.filter(HermesUserMemory.enabled == 1)
        rows = (
            query.order_by(HermesUserMemory.updated_at.desc(), HermesUserMemory.id.desc())
...
def set_user_memory_enabled(memory_uid: str, enabled: bool) -> bool:
    ...
    ensure_hermes_memory_tables()
    with get_session() as session:
        row = session.query(HermesUserMemory).filter(HermesUserMemory.memory_uid == uid).first()
        if not row:
            return False
...
def _serialize_activity(row: HermesMachineActivity) -> dict[str, Any]:
    return {
        "id": row.id,
...
def record_machine_activity(
    *,
    ...
    with get_session() as session:
        row = (
            session.query(HermesMachineActivity)
            .filter(HermesMachineActivity.content_hash == content_hash)
            .filter(HermesMachineActivity.compacted == 0)
            .first()
        )
        is_new = row is None
        if row:
...
def list_machine_activity(
    *,
    ...
    ensure_hermes_memory_tables()
    with get_session() as session:
        query = session.query(HermesMachineActivity)
        if surface:
            query = query.filter(HermesMachineActivity.surface == surface.strip().lower())
        if not include_compacted:
            query = query.filter(HermesMachineActivity.compacted == 0)
        rows = (
            query.order_by(HermesMachineActivity.last_seen_at.desc(), HermesMachineActivity.id.desc())
    ...
    *,
    ...
    with get_session() as session:
        rows = (
            session.query(HermesMachineActivity)
            .filter(HermesMachineActivity.compacted == 0)
            .filter(HermesMachineActivity.last_seen_at < cutoff)
            .order_by(HermesMachineActivity.last_seen_at.asc(), HermesMachineActivity.id.asc())
            .all()
        )
        groups: dict[tuple[Any, ...], list[HermesMachineActivity]] = defaultdict(list)
        for row in rows:
        ...
        for (day, surface, app_name, workspace_path, project_id), group in groups.items():
            ...
            existing = (
                session.query(HermesMachineActivity)
                .filter(HermesMachineActivity.content_hash == content_hash)
                .filter(HermesMachineActivity.compacted == 1)
                .first()
            )
            if existing:
                ...
                existing = HermesMachineActivity(
...
def run_weekly_machine_activity_compaction(
    *,
    ...
    with get_session() as session:
        state = (
            session.query(HermesMaintenanceState)
            .filter(HermesMaintenanceState.key == "machine_activity_weekly_compaction")
            .first()
        )
    ...
    with get_session() as session:
        ...
        if state:
            ...
            session.add(HermesMaintenanceState(key="machine_activity_weekly_compaction", value_json=_json_dumps(value)))
        session.commit()
