...
def build_standard_loop_steps(
    *,
    ...
    check_instruction: str | None = None,
) -> list[dict[str, Any]]:
    """Build the canonical 4-step elorm loop used when generating bundle JSON."""
    shared_guardrail = _guardrail_text(loop_contract)
    skills = list(ARCHETYPE_SKILL_BUNDLES.get(archetype) or [])
    check_tools, check_other = _check_tools(check_command)
    work_instruction = work_instruction or f"Work toward: {goal}. {step_1}"
    check_instruction = check_instruction or f"Run: {check_command}"

    return [
        {
            "name": "Do iteration work",
            "instruction": work_instruction,
            "guardrail": shared_guardrail,
            "failure_checklist": [
                "No concrete evidence (diff, test output, or UI proof)",
                "Changes outside ticket scope or unrelated refactors",
            ],
            "validation_prompt": "Iteration work produced concrete evidence (diff, test output, or UI proof).",
            "validation_type": "llm_judgment",
            "validation_pass_action": "next_step",
            "validation_fail_action": "retry_loop",
            "skills": skills,
            "tools": ["cli"],
        ...
        },
        {
            ...
            "guardrail": "- Do not modify the check command to force a pass.\n" + shared_guardrail,
            "failure_checklist": [
                "Check command was changed to force a pass",
                "Check was skipped or bypassed",
                "Exit code non-zero without documented blocker",
            ],
            "validation_prompt": f"{check_command} completes successfully (exit 0 or explicit pass criteria).",
            "validation_type": "exit_code" if check_other else "llm_judgment",
            "validation_pass_action": "next_step",
            "validation_fail_action": "retry_loop",
            "on_fail_goto_position": 0,
            "skills": skills,
            "tools": check_tools,
            "other_tool": check_other,
            "action_type": "run_command" if check_other else "playwright",
            "command": check_command if check_other else None,
        },
        {
            "name": "Evaluate exit",
            "instruction": f"Decide if exit condition is met: {exit_when}",
            "guardrail": "- Do not game metrics or weaken exit criteria.\n" + shared_guardrail,
            "failure_checklist": [
                "Exit criteria were weakened to force completion",
                "Max iterations reached without escalation",
                "Evidence does not support the exit decision",
            ],
            "validation_prompt": f"Exit condition satisfied: {exit_when}",
            "validation_type": "llm_judgment",
            "validation_pass_action": "next_step",
            "validation_fail_action": "retry_loop",
            "on_fail_goto_position": 0,
        ...
        {
            "name": "Report",
            "instruction": "Summarize iteration outcome for the orchestrator and ticket.",
            "guardrail": shared_guardrail,
            "failure_checklist": [
                "Report missing status or evidence",
                "Unclear whether another iteration is needed",
            ],
            "validation_prompt": "Report includes status, evidence, and whether another iteration is needed.",
            "validation_type": "llm_judgment",
