...
def _project_handoff_plan(source_surface: str, instruction: str) -> DelegatedPlan:
    ...
    return DelegatedPlan(
        ...
        original_instruction=instruction,
        steps=[
            DelegatedStep("resolve_project_context", "hermes_memory", ["user_clarification"], "Find the active project, ticket, or workflow context.", verifies=["project_context_found"]),
            DelegatedStep("prepare_handoff_packet", "hermes", ["workflow_ticket"], "Build a redacted Decisions-to-worker packet.", verifies=["handoff_packet_created"]),
            DelegatedStep(
...
def _browser_workflow_plan(source_surface: str, instruction: str) -> DelegatedPlan:
    return DelegatedPlan(
        ...
        original_instruction=instruction,
        steps=[
            DelegatedStep("prepare_browser_task", "hermes_orchestrator", ["user_clarification"], "Resolve URL, browser state, credentials, and expected result.", verifies=["browser_task_actionable"]),
...
def _general_plan(source_surface: str, instruction: str) -> DelegatedPlan:
    return DelegatedPlan(
        ...
        original_instruction=instruction,
        steps=[
            DelegatedStep("clarify_goal", "hermes_memory", ["user_clarification"], "Resolve the target account, project, app, and expected output.", verifies=["goal_is_actionable"]),
            DelegatedStep("choose_execution_route", "hermes_orchestrator", ["user_clarification"], "Select API, browser, desktop, or project handoff route.", verifies=["route_selected"]),
        ],
