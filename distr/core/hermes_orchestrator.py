"""Hermes hybrid orchestrator — policy-first routing with optional LLM override."""
...
def _call_orchestrator_llm(
    *,
    ...
    try:
        from distr.core.hermes import get_hermes_role_model
        from distr.core.settings import load_settings_from_db

        settings = load_settings_from_db()
        if not bool(settings.get("hermes_orchestrator_routing_enabled", True)):
            return None
        if str(policy.get("routing_mode") or "hybrid").lower() == "policy":
            return None

        provider, model = get_hermes_role_model("orchestrator")
        if not provider and not model:
            return None

        from distr.core.skills.catalog import hermes_skill_catalog
...
def _determine_source(baseline: dict[str, str], final: dict[str, str], board: Any | None) -> RouteSource:
    if final.get("backend") != baseline.get("backend") or final.get("model") != baseline.get("model"):
        if board is not None:
            from distr.core.hermes import normalize_board_hermes_policy
...
def resolve_execution_route(
    *,
    ...
    allow_hermes_override: bool = True,
    ...
    policy = normalize_board_hermes_policy(getattr(board, "hermes_policy", None) if board else None)
