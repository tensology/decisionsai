...
def get_active_project_context() -> Optional[str]:
    ...
    try:
        ...
        if project.folder_location:
            ...
            context_parts.append(f"✅ Use self_update_via_cursor first for bug fixes and self-update requests (developer mode only).")
            context_parts.append(f"✅ If self_update_via_cursor is unavailable/disabled, use create_project_ticket for work instructions.")
            context_parts.append(f"")
