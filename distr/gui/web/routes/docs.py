...
def create_routes(base_path: str = "") -> APIRouter:
  ...
  -d '{{"provider": "ollama", "model_name": "qwen3:8b", "title": "My Chat", "starting_question": "Hello!"}}'""",
                        "body": {
                            ...
                            "provider": {"type": "string", "required": False, "description": "LLM provider: ollama, openai, anthropic, groq, openrouter, kilocode"},
                            "model_name": {"type": "string", "required": False, "description": "Model name (e.g. qwen3:8b, gpt-4o)"},
                            "voice_provider": {"type": "string", "required": False, "description": "TTS provider: kokoro, openai, elevenlabs, qwen3"},
  ...
  -d '{{"done": true}}'""",
            ...
            # ── Voices & TTS ───────────────────────────────────
            {
                ...
                "endpoints": [
                    ...
                    {"method": "GET", "path": "/api/voices/qwen3", "summary": "List Qwen3-TTS voices", "description": "Returns Qwen3-TTS preset voices.", "curl": f'curl -s {base}/api/voices/qwen3', "body": None, "response_example": '[{"id": "Aiden", "name": "Aiden (sunny American male)"}]'},
                    {"method": "POST", "path": "/api/play-voice", "summary": "Preview a voice", "description": "Generates a short TTS sample and returns it as an MP3 file.", "curl": f"curl -s -X POST {base}/api/play-voice -H 'Content-Type: application/json' -d '{{\"provider\": \"kokoro\", \"voice\": \"af_heart\", \"speed\": 1.0}}' --output sample.mp3", "body": {"provider": {"type": "string", "required": True, "description": "TTS provider: kokoro, elevenlabs, openai, qwen3"}, "voice": {"type": "string", "required": True, "description": "Voice ID"}, "speed": {"type": "float", "required": False, "description": "Playback speed 0.5-2.0 (default 1.0)"}, "voice_name": {"type": "string", "required": False, "description": "Display name (ElevenLabs fallback)"}}, "response_example": "(binary MP3 audio)"},
                ],
            },
            # ── Custom Voice Cloning ───────────────────────────
            {
                ...
                "endpoints": [
                    {"method": "GET", "path": "/api/custom-voices", "summary": "List custom voices", "description": "Returns all custom cloned voices, optionally filtered by provider. Status: pending, processing, ready, or failed.", "curl": f'curl -s "{base}/api/custom-voices?provider=kokoro"', "params": [{"name": "provider", "type": "string", "required": False, "description": "Filter: kokoro, elevenlabs, qwen3"}], "body": None, "response_example": '[{"id": 1, "name": "My Voice", "provider": "kokoro", "status": "ready", "provider_voice_id": "custom_1"}]'},
                    {"method": "POST", "path": "/api/custom-voices", "summary": "Clone a voice", "description": "Create a custom voice clone by uploading audio samples (multipart form). Cloning runs in background; poll status endpoint. Providers: kokoro (Kanade offline), elevenlabs (IVC API, max 5), qwen3 (voice prompt baking).", "curl": f"curl -s -X POST {base}/api/custom-voices -F 'name=My Voice' -F 'provider=kokoro' -F 'personality=cheerful' -F 'gender=female' -F 'audio=@sample.wav'", "body": {"name": {"type": "string", "required": True, "description": "Voice name"}, "provider": {"type": "string", "required": True, "description": "kokoro, elevenlabs, or qwen3"}, "system_prompt": {"type": "string", "required": False, "description": "Voice description"}, "personality": {"type": "string", "required": False, "description": "Personality description"}, "gender": {"type": "string", "required": False, "description": "male or female (default: female)"}, "audio": {"type": "file", "required": True, "description": "Audio files (.wav, .mp3, .m4a, .ogg, .flac, .webm)"}}, "response_example": '{"id": 3, "status": "processing", "name": "My Voice"}'},
            ...
            # ── Settings ───────────────────────────────────────
            {
                ...
                "endpoints": [
                    ...
                    {"method": "POST", "path": "/api/general", "summary": "Save general settings", "description": "Saves all general settings and emits live-update signals for oracle, voice, and playback changes.", "curl": f"curl -s -X POST {base}/api/general -H 'Content-Type: application/json' -d '{{\"voice_provider\": \"kokoro\", \"playback_speed\": 1.2, \"selected_oracle\": \"0.gif\"}}'", "body": {"voice_provider": {"type": "string", "required": False, "description": "kokoro, elevenlabs, openai, qwen3, coqui"}, "playback_speed": {"type": "float", "required": False, "description": "0.5-2.0"}, "speech_volume": {"type": "int", "required": False, "description": "0-100"}, "selected_oracle": {"type": "string", "required": False, "description": "Oracle skin filename"}, "sphere_size": {"type": "int", "required": False, "description": "Oracle size 4-10"}, "oracle_position": {"type": "string", "required": False, "description": "custom, top_left, top_right, etc."}}, "response_example": '{"success": true, "message": "Settings saved and oracle updated"}'},
